...
### FR-9: AI Daily Summary (Gemini 기반 개인화 코칭)
**Priority**: High | **MVP**: Yes | **Demo Focus**: ⭐

Gemini API가 오늘 루틴 결과와 과거 데이터를 분석하여 개인화된 코칭 메시지를 생성한다.

#### 배경 및 목적
- 단순 통계 표시 → AI가 분석한 인사이트 제공
- 해커톤 심사 기준 "Gemini 활용" 강화
- 사용자 동기부여 및 습관 형성 지원

| ID | Requirement |
|----|-------------|
| FR-9.1 | 세션 종료 시 백엔드 API 호출하여 AI Summary 생성 요청 |
| FR-9.2 | 백엔드에서 Gemini API로 개인화된 요약 생성 |
| FR-9.3 | 오늘 루틴 결과 (완료/스킵/소요시간) 분석 |
| FR-9.4 | 최근 7일 히스토리 분석 (연속 완료, 스킵 패턴 등) |
| FR-9.5 | 칭찬 + 개선 포인트 + 팁 형식의 코칭 메시지 생성 |
| FR-9.6 | 생성된 aiSummary를 DailyReport에 저장 |
| FR-9.7 | Report 페이지에서 AI Summary 텍스트 표시 |
| FR-9.8 | Listen 버튼으로 TTS 음성 재생 (Web Speech API) |
| FR-9.9 | Share 버튼으로 Summary 카드 이미지 공유 (html2canvas) |
...
```
당신은 친근한 아침 루틴 코치입니다. 사용자의 오늘 루틴 결과와 최근 7일 기록을 분석하여 
개인화된 코칭 메시지를 작성해주세요.
...
```
"오늘 4개 중 3개 완료, 75% 달성! 👏 
스트레칭 7일 연속 완료 중이에요, 정말 대단해요! 🔥
요가는 최근 3일 스킵이 많았는데, 시간이 부담되면 5분 미니 요가로 시작해보는 건 어때요? 
내일도 화이팅! 💪"
```
