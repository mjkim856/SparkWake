...
/**
 * Create and initialize a Gemini Live session for real-time audio, video, transcription, and function-calling tool interactions.
 *
 * @param ephemeralToken - Ephemeral API key used to authenticate with the v1alpha Live API
 * @param systemInstruction - System prompt applied to the model for the session
 * @param callbacks - Lifecycle and event handlers for the session (open, message, audio, error, close, interrupt, tool call)
 * @returns A LiveSession object with methods to send text, send audio, send video, send tool responses, close the session, and query connection state
 */
